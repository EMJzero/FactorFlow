...
def mapperForcedSettingsUpdate(arch : Arch, verbose : bool = True) -> None:
    if sum(1 for l in arch if isinstance(l, MemLevel)) < 12:
...
"""
def filterEquiDataflowPerms(arch : Arch, level : MemLevel, perms : list[list[str]], in_matters : bool = True, w_matters : bool = True, out_matters : bool = True) -> list[list[str]]:
    """
    ...
    def dimsAtOne(level : MemLevel) -> dict[str, bool]:
        return {dim: level.factors.dimProduct(dim) == 1 for dim in arch.coupling.dims}
    ...
    for perm in perms:
        ...
        for unique_perm in unique_perms:
            ...
            i, j = next((i for i, dim in enumerate(perm_effective) if dim in arch.coupling.flat_in_coupling), 0), next((j for j, dim in enumerate(unique_perm_effective) if dim in arch.coupling.flat_in_coupling), 0)
            input_ok = not in_matters or i == j and set(perm_effective[:i]) == set(unique_perm_effective[:j]) and ((perm_effective[i] == unique_perm_effective[j] or (not arch.coupling.getDimSum('in', perm_effective[i], 2) and not arch.coupling.getDimSum('in', unique_perm_effective[j], 2))) or (not level.multiple_reuses and i != 0))
            if input_ok:
                i, j = next((i for i, dim in enumerate(perm_effective) if dim in arch.coupling.flat_w_coupling), 0), next((j for j, dim in enumerate(unique_perm_effective) if dim in arch.coupling.flat_w_coupling), 0)
                weights_ok = not w_matters or i == j and set(perm_effective[:i]) == set(unique_perm_effective[:j]) and ((perm_effective[i] == unique_perm_effective[j] or (not arch.coupling.getDimSum('w', perm_effective[i], 2) and not arch.coupling.getDimSum('w', unique_perm_effective[j], 2))) or (not level.multiple_reuses and i != 0))
                if weights_ok:
                    i, j = next((i for i, dim in enumerate(perm_effective) if dim in arch.coupling.flat_out_coupling), 0), next((j for j, dim in enumerate(unique_perm_effective) if dim in arch.coupling.flat_out_coupling), 0)
                    output_ok = not out_matters or i == j and set(perm_effective[:i]) == set(unique_perm_effective[:j]) and ((perm_effective[i] == unique_perm_effective[j] or (not arch.coupling.getDimSum('out', perm_effective[i], 2) and not arch.coupling.getDimSum('out', unique_perm_effective[j], 2))) or (not level.multiple_reuses and i != 0))
                    if output_ok:
                        eq_match = True
                        break
        if not eq_match:
...
def pickBestPermsIteratively(arch : Arch) -> None:
    ...
    for i in range(len(mem_levels)):
        ...
        if sum(1 for dim in arch.coupling.dims if level.factors.dimProduct(dim) > 1) <= 1: # no iterations or a single dimension is iterated, permutations don't matter
            continue
        ...
        candidate_perms = filterEquiDataflowPerms(arch, level, candidate_perms, in_matters, w_matters, out_matters)
...
def factorFlow(arch : Arch, comp : Shape, bias_read : bool, verbose : bool = False) -> tuple[Arch, float, int]:
    ...
    """
    def exploreOneStep(remaining_steps : int = 1, target_dst_level_idx : Optional[int] = None, freeze_memories : bool = False, freeze_spatials : bool = False, freeze_perms : bool = False, only_flow_inward : bool = True, iterate_amounts : bool = False, limit_n_dst_to_c_src : bool = False) -> dict[tuple[Union[int, str]], float]:
        choices = {}
    ...
    def localSearch(initial_steps_to_explore : int = 1, final_steps_to_explore : int = 1, steps_to_explore_increment : int = 1, freeze_memories : bool = False, freeze_spatials : bool = False, freeze_perms : bool = False, iterate_amounts : bool = False, limit_n_dst_to_c_src : bool = False) -> None:
        ...
        while not Settings.forced_termination_flag:
            choices = exploreOneStep(remaining_steps = steps_to_explore, freeze_spatials = freeze_spatials, freeze_memories = freeze_memories, freeze_perms = freeze_perms, only_flow_inward = only_flow_inward, iterate_amounts = iterate_amounts, limit_n_dst_to_c_src = limit_n_dst_to_c_src)
            # >>> GREEDY MOVE <<<
    ...
    if not already_initialized:
        if Settings.LOCAL_SEARCH_SPATIAL_LEVELS:
            ...
            localSearch(initial_steps_to_explore = Settings.SPATIAL_STEPS_TO_EXPLORE, final_steps_to_explore = Settings.SPATIAL_STEPS_TO_EXPLORE, freeze_memories = True, freeze_spatials = False, freeze_perms = True, iterate_amounts = Settings.SPATIAL_ITERATE_AMOUNTS, limit_n_dst_to_c_src = Settings.SPATIAL_LIMIT_NEXT_STEP_DST_TO_CURRENT_SRC) # optimize spatial levels, may only remove factors from memory levels
        else:
...
"""
Compute meaningful permutations to explore for each memory level.
...
def optimizeDataflows(arch : Arch, comp : Shape, bias_read : bool, thread_idx : int = -1, threads_count : int = 1, past_perms : dict[tuple[int, ...], ThreadSafeHeap[float, list[LevelCore], int, int]] = None, lock : threading.Lock = None, barrier : threading.Barrier = None, verbose : bool = False) -> Optional[tuple[Arch, float]]:
    ...
    arch, wart, _ = factorFlow(arch, comp, bias_read, verbose)
    if thread_idx == -1:
        return arch, wart
    elif thread_idx == 0:
