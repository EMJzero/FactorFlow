...
def mapperForcedSettingsUpdate(arch : Arch, verbose : bool = True) -> None:
    if sum(1 for l in arch if isinstance(l, MemLevel)) < 6:
...
def factorFlow(arch : Arch, comp : Shape, bias_read : bool, verbose : bool = False) -> tuple[Arch, float, int]:
    ...
    """
    def exploreOneStepFurther(choices : dict[tuple[Union[int, str], ...], float], remaining_steps : int = 1, freeze_memories : bool = False, freeze_spatials : bool = False, only_flow_inward : bool = True, iterate_amounts : bool = False, limit_n_dst_to_c_src : bool = False) -> dict[tuple[Union[int, str], ...], float]:
        further_choices = {}
        for choice, wart in choices.items():
            if remaining_steps >= 1:
                ...
                nested_choices = exploreOneStep(remaining_steps, freeze_memories = freeze_memories, freeze_spatials = freeze_spatials, only_flow_inward = only_flow_inward, iterate_amounts = iterate_amounts, limit_n_dst_to_c_src = limit_n_dst_to_c_src)
                if len(nested_choices) > 0:
                    # >>> GREEDY MOVE <<<
                    best_choice = max(nested_choices, key=nested_choices.get)
                    if nested_choices[best_choice] >= wart:
    ...
    def localSearch(initial_steps_to_explore : int = 1, final_steps_to_explore : int = 1, steps_to_explore_increment : int = 1, freeze_memories : bool = False, freeze_spatials : bool = False, iterate_amounts : bool = False, limit_n_dst_to_c_src : bool = False) -> None:
        ...
        while True:
            if len(choices) == 0:
                ...
                # TODO: issue, since we overwrite each time 'choices' with those one further step away, once 'only_flow_inward' becomes False
                #       we don't have anymore the initial mapping in 'choices' and can't try to move factors backwards from it...
                choices = exploreOneStepFurther(choices, remaining_steps = steps_to_explore - initial_steps_to_explore, freeze_spatials = freeze_spatials, freeze_memories = freeze_memories, only_flow_inward = only_flow_inward, iterate_amounts = iterate_amounts, limit_n_dst_to_c_src = limit_n_dst_to_c_src)
            # >>> GREEDY MOVE <<<
    ...
    if not already_initialized:
        ...
        #if Settings.STEPS_TO_EXPLORE > 1:
        #    localSearch(final_steps_to_explore = Settings.STEPS_TO_EXPLORE, freeze_memories = False, freeze_spatials = True, iterate_amounts = True) # keep a single iteration on spatial levels, optimize memory levels
        if Settings.LOCAL_SEARCH_SPATIAL_LEVELS:
